...
public class Crypto(private val cryptoProvider: CryptoProvider) {
    ...
    public fun hash(message: String, size: Int): Result<ByteArray> =
        hash(message.toByteArray(), size)

    public fun hash(message: HexString, size: Int): Result<ByteArray> =
        hash(message.toByteArray(), size)

    public fun hash(message: ByteArray, size: Int): Result<ByteArray> =
