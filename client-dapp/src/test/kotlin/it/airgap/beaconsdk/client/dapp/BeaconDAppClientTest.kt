...
//
//            val versionedResponses = mock.beaconVersionedResponses(dAppVersion, walletId, dependencyRegistry.mock.getVersionedBeaconMessageContext).shuffled().first()
//            val connectionResponseMessage = BeaconIncomingConnectionMessage(origin, versionedResponses)
//
//            val disconnectMessage = mock.disconnectBeaconMessage(senderId = dAppId, destination = origin)
//            val versionedDisconnectMessage = VersionedBeaconMessage.from(disconnectMessage.senderId, disconnectMessage, dependencyRegistry.mock.getVersionedBeaconMessageContext)
//            val connectionDisconnectMessage = BeaconIncomingConnectionMessage(disconnectMessage.destination, versionedDisconnectMessage)
//
//            val beaconMessageFlow = mock.beaconConnectionMessageFlow(2)
//            every { connectionController.subscribe() } answers { beaconMessageFlow }
//
//            beaconDAppClient.connect()
//                .onStart { beaconMessageFlow.mock.tryEmitValues(listOf(connectionDisconnectMessage, connectionResponseMessage)) }
//                .mapNotNull { it.getOrNull() }
//                .take(1)
//                .single()
//
